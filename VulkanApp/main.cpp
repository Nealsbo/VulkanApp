...
private:
    void InitWindow() {
        ...
        window = glfwCreateWindow(screenWidth, screenHeight, "VulkanApp", nullptr, nullptr);
    ...
    void CreateInstance() {
        ...
        appInfo.pEngineName = "VulkanEnging";
    ...
    void CreateSurface() {
        ...
        int res = glfwCreateWindowSurface(instance, window, nullptr, &surface);
        if (res != VK_SUCCESS) {
            std::cout << "ERROR: Failed to create window surface!\n";
            exit(-1);
        }
    }
    ...
    void PickPhysicalDevice() {
        ...
        if (deviceCount == 0) {
            std::cout << "ERROR: Enum devices!\n";
            exit(-1);
        }

        std::cout << "INFO: Physical device count: " << deviceCount << "\n";
    ...
    void CreateLogicalDevice() {
        ...
        int res = vkCreateDevice(physicalDevice, &createInfo, nullptr, &device);
        if (res != VK_SUCCESS) {
    ...
    void CreateRenderPass() {
        ...
        int res = vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass);
        if( res != VK_SUCCESS) {
    ...
    void CreateGraphicsPipeline() {
        ...
        int res = vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout);
        if( res != VK_SUCCESS) {
    ...
    void CreateFramebuffers() {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            ...
            int res = vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]);
            if( res != VK_SUCCESS ) {
    ...
    void CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
        ...
        int res = vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory);
        if (res != VK_SUCCESS) {
    ...
    uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
        ...
        std::cout << "ERROR: Failed to find suitable memory type!\n";
        exit(-1);
    }
    ...
    void CreateCommandBuffer() {
        ...
        int res = vkAllocateCommandBuffers(device, &allocInfo, commandBuffer.data());
        if( res != VK_SUCCESS ) {
            std::cout << "ERROR: Failed to allocate command buffers!\n";
            exit(-1);
        }
    }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        if( res != VK_SUCCESS ) {
            std::cout << "ERROR: Failed to begin recording command buffer!\n";
            exit(-1);
        }
        ...
        if( res != VK_SUCCESS ) {
            std::cout << "ERROR: Failed to record command buffer!\n";
            exit(-1);
        }
    }
    ...
    void CreateSyncObjects() {
        ...
        for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphore[i]) != VK_SUCCESS ||
                ...
                std::cout << "ERROR: Failed to create semaphores!\n";
                exit(-1);
            }
        }
    }
    ...
    void DrawFrame() {
        ...
        uint32_t imageIndex;
        int res = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphore[currentFrame], VK_NULL_HANDLE, &imageIndex);
        ...
        if (res == VK_ERROR_OUT_OF_DATE_KHR) {
            ...
            std::cout << "ERROR: Failed to acquire swap chain image!\n";
            exit(-1);
        }
        ...
        if( res != VK_SUCCESS ) {
            std::cout << "ERROR: Failed to submit draw command buffer!\n";
            exit(-1);
        }
        ...
        if (res == VK_ERROR_OUT_OF_DATE_KHR || res == VK_SUBOPTIMAL_KHR || framebufferResized) {
            ...
            std::cout << "ERROR: Failed to present swap chain image!\n";
        }
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) {
        ...
        int res = vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule);
        if ( res != VK_SUCCESS ) {
